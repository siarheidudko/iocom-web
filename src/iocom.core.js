...
function SendFileToInternalFS(files, ParamOne, ParamFour, ParamSix, ParamSeven, ParamNine, timeOnCompl, ParamEight, ParamTwelve){
	var result = new Promise(function(resolve, reject){
		if((typeof(window.files) === 'object') && (typeof(files[0]) === 'object')){
			...
			var serverlink = window.location.protocol.substr(0,window.location.protocol.length - 1) + '://' + window.location.hostname + ':' + window.location.port + '/v2/upload';
			fetch(serverlink, {
				method: "POST",
	...
	result.then(
		function(resolve){
			var _resolve;
			try{
				_resolve = JSON.parse(resolve).status;
			}catch(e){}
			if(_resolve === 'upload'){
